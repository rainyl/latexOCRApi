...
def token2str(tokens, tokenizer):
    ...
    return [
        ''.join(detok.split(' ')).replace('Ġ', ' ').replace('[EOS]', '').replace('[BOS]', '').replace('[PAD]', '').strip() 
        for detok in dec
    ]
...
def verifyUser(user: str, token: str):
    if user in VALID_TOKENS:
        if token in VALID_TOKENS[user]:
            return True
    return False
